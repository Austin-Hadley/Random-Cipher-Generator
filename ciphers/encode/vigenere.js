// create a vigenere cipher
// use the ceaser cipher to create a vigenere cipher

const ceaser = require('./ceaser');

// run the function x times
function vigenereCipher(string, x) {
    // create a variable to store the new cipher
    var newCipher = string;
    // loop through the string
    for (var i = 0; i < x; i++) {
        // create a variable to store the shift value
        var shift = ceaser.randShift();
        // loop through the string
        for (var i = 0; i < newCipher.length; i++) {
            // get the character code of the current character
            var charCode = newCipher.charCodeAt(i);
            // if the character code is between 65 and 90 (A-Z)
            if (charCode >= 65 && charCode <= 90) {
                // if the character code is greater than 77 (M)
                if (charCode > 77) {
                    // subtract 13 from the character code
                    charCode -= shift;
                } else {
                    // add 13 to the character code
                    charCode += shift;
                }
            // if the character code is between 97 and 122 (a-z)
            } else if (charCode >= 97 && charCode <= 122) {
                // if the character code is greater than 109 (m)
                if (charCode > 109) {
                    // subtract 13 from the character code
                    charCode -= shift;
                } else {
                    // add 13 to the character code
                    charCode += shift;
                }
            }
            // add the new character to the new cipher
            newCipher += String.fromCharCode(charCode);
        }
    }
    // return the new cipher
    return newCipher;
}

// export the function
module.exports = vigenereCipher();
